...
public abstract class AbstractDoubleArrayTrie {
	...
	/**
	 * After a state conflict and after the moved state's children have been themselves moved, the base of the state must change. This
	 * method is called AFTER this change happens and after all children have been moved
	 * 
	 * @param stateIndex
	...
	/**
	 * Ensures that the size of the backing store is enough to have an addressable <tt>index</tt>. This means that all array accesses up to
	 * and including <tt>index</tt> must be successful. The getSize() call must return the greatest value that has been passed as an
	 * argument to this method during a run.
	 * 
	 * @param index
