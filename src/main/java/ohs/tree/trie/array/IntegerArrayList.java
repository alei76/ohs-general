...
/**
 * A bare bones Array List implementation specifically designed for storing integer values. It has half the memory requirements of an
 * ArrayList&lt;Integer&gt;, no pointer dereferencing for fetching the values while maintaining the access speed of an ArrayList. It is
 * perfect for creating create-once-read-many large lists of integers. It does not implement any of the Collections Framework interfaces,
 * since this is not a library class but an underlying store for higher lever structures and the added code complexity was not necessary. It
 * should be simple however to extend and add whatever methods you need. Obviously however, generics are out of the question.
 * 
 * Of course I have been influenced by the implementation in the Sun JDK but I have tried to keep true to myself. However, there aren't many
 * ways to implement something as simple as an ArrayList.
 * 
 * @author Chris Gioran
 */
public class IntegerArrayList implements IntegerList {
	...
	/**
	 * Package protected constructor for use by factories. Besides the initial capacity, this constructor allows for specifying the growth
	 * characteristics. Whenever an increase of the storing array is needed, its new size is calculated as
	 * 
	 * <p>
	...
	/**
	 * Ensures that this instance has enough capacity to hold at least <tt>capacity</tt> values. Runs in O(n) worst case, O(1) amortized.
	 * 
	 * @param capacity
	...
	/**
	 * Checks if the given index is less than the size of this ArrayList. The other half of the check (if it is &lt;0) is performed by the
	 * array implementation. This check is needed because the array can be larger that <tt>size</tt> so accesses beyond that will be allowed
	 * by the JVM.
	 */
