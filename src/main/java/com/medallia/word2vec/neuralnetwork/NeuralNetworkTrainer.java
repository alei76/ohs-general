...
public abstract class NeuralNetworkTrainer {
	...
	abstract class Worker extends CallableVoid {
		...
		public void run() throws InterruptedException {
			for (int[] sent : batch) {
				...
				for (int w : sent) {
					if (!huffmanNodes.containsKey(w))
						continue;

					wordCount++;
					if (config.downSampleRate > 0) {
						HuffmanNode huffmanNode = huffmanNodes.get(w);
		...
		private void updateAlpha(int iter) {
			...
			listener.update(Stage.TRAIN_NEURAL_NETWORK, currentActual / (double) (config.iterations * numTrainedTokens + 1));
		}
	}
	...
	/**
	 * This contains the outer layers of the neural network First dimension is the vocab, second is the layer
	 */
	final double[][] syn0;
