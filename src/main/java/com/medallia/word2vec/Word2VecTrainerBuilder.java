 ...
 * <li>The original version injects a &lt;/text&gt; token into the vocabulary (with a word cnt of 0) as a substitute for newlines in the
 * input file. This Java port'text vocabulary excludes the token.
 * </ul>
 * <li>In partitioning the file for processing
 * <ul>
 * <li>The original version assumes that sentences are delimited by newline characters and injects a sentence boundary per 1000 non-filtered
 * tokens, i.e. valid token by the vocabulary and not removed by the randomized sampling process. Java port mimics this behavior for now ...
 * <li>When the original version encounters an empty line in the input file, it re-processes the first word of the last non-empty line with
 * a sentence length of 0 and updates the random value. Java port omits this behavior.
 * </ul>
 * <li>In the sampling function
 * <ul>
 * <li>The original C documentation indicates that the range should be between 0 and 1e-5, but the default value is 1e-3. This Java port
 * retains that confusing information.
 * <li>The random value generated for comparison to determine if a token should be filtered uses a float. This Java port uses double
 * precision for twice the fun
 * </ul>
...
public class Word2VecTrainerBuilder {
	...
	public interface TrainingProgressListener {
		 ...
		 * 
		 * @param stage
		 *            Current {@link Stage} of processing
		 * @param progress
		 *            Progress of the current stage as a double value between 0 and 1
		 */
	...
	/**
	 * Set threshold for occurrence of words. Those that appear with higher frequency in the training data, e.g. stopwords, will be randomly
	 * removed
	 * <p>
	...
	/**
	 * Specify the minimum frequency for a valid token to be considered part of the vocabulary
	 * <p>
	 * Defaults to 5
	 */
	 ...
	 * 
	 * @param vocab
	 *            {@link Map} from token to frequency
	 */
